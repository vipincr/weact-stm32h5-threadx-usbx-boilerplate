...
build_project() {
    ...
    local axf_path="$BUILD_DIR/$preset/${PROJECT_NAME}.axf"
    local bin_path="$BUILD_DIR/$preset/${PROJECT_NAME}.bin"

    if [ ! -f "$axf_path" ]; then
        echo -e "${RED}[ERROR]${NC} AXF not found: $axf_path"
        return 1
    fi

    if command -v "$ARM_OBJCOPY" >/dev/null 2>&1; then
        "$ARM_OBJCOPY" -O binary "$axf_path" "$bin_path" || return 1
        "$ARM_OBJCOPY" -O ihex "$axf_path" "$BUILD_DIR/$preset/${PROJECT_NAME}.hex" || return 1
...
flash_project() {
    local preset="$1"
    local axf_path="$BUILD_DIR/$preset/${PROJECT_NAME}.axf"
    local image="$axf_path"

    if [ ! -f "$image" ]; then
        echo -e "${RED}[ERROR]${NC} AXF not found: $image"
        return 1
    fi
