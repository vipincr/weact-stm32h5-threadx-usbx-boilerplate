                                   ...
                                   uint32_t pConf, uint32_t *Sze)
{
  ...
  *Sze += (uint32_t)sizeof(USBD_CDCUnionFuncDescTypedef);
  ...
  /* Append Endpoint descriptor to Configuration descriptor (CMD Endpoint) */
  ...
  __USBD_FRAMEWORK_SET_IF(pdev->tclasslist[pdev->classId].Ifs[1], 0U, 2U, 0x0AU,
                          0U, 0U, 0U);

  /* Append Endpoint descriptor to Configuration descriptor (OUT Endpoint) */
  __USBD_FRAMEWORK_SET_EP((pdev->tclasslist[pdev->classId].Eps[0].add),
                          (USBD_EP_TYPE_BULK),
                          (uint16_t)(pdev->tclasslist[pdev->classId].Eps[0].size),
                          (0x00U), (0x00U));

  /* Append Endpoint descriptor to Configuration descriptor (IN Endpoint) */
  __USBD_FRAMEWORK_SET_EP((pdev->tclasslist[pdev->classId].Eps[1].add),
                          (USBD_EP_TYPE_BULK),
                          (uint16_t)(pdev->tclasslist[pdev->classId].Eps[1].size),
                          (0x00U), (0x00U));
